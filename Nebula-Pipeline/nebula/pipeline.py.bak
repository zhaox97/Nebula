...
class Pipeline:
    ...
    def delete_model(self, index):
        ...
        if index not in range(len(self._models)):
    ...
    def start(self, argv=None):
        ...
        if ATTRIBUTES in setup_data:
            Model.global_weight_vector = {attr: 1.0/len(setup_data[ATTRIBUTES])  for attr in setup_data[ATTRIBUTES].keys()}
            # for SIRIUS
            Model.global_attribute_weight_vector = {attr: 1.0/len(setup_data[ATTRIBUTES]) for attr in setup_data[ATTRIBUTES].keys()}
        
        if   DOCUMENTS_LIST  in setup_data:
             Model.global_document_weight_vector ={doc: 1.0/len(setup_data[DOCUMENTS_LIST]) for doc in setup_data[DOCUMENTS_LIST].keys()}
